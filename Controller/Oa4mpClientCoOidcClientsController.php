...
//      $d['sort_key_template'] = $d['sort_key_template'] ?? $adminClient['DefaultDynamoConfig']['sort_key_template'];
      ...
      $args['deep'] = true;
      // TODO
      $this->log("FOO about to save data " . print_r($data, true));
      $ret = $this->Oa4mpClientCoOidcClient->saveAssociated($data, $args);

      if(!$ret) {
        // TODO
        $this->log("FOO validationErrors is " . print_r($this->Oa4mpClientCoOidcClient->validationErrors, true));
...
//
  ...
  function edit($id) {
    $curdata = $this->Oa4mpClientCoOidcClient->current($id);
    ...
    $adminClient = $this->Oa4mpClientCoOidcClient->admin($id);
    ...
    $oa4mpServer = new Oa4mpClientOa4mpServer();
    $synchronized = $oa4mpServer->oa4mpVerifyClient($adminClient, $curdata);
    if(!$synchronized) {
...
//      } 
      ...
      $ret = $oa4mpServer->oa4mpEditClient($adminClient, $curdata, $data);
      if($ret == 0) {
...
//      }
      ...
      $this->log("FOO validation errors is " . print_r($this->Oa4mpClientCoOidcClient->validationErrors, true));
...
//    // TODO
//    $this->log("FOO marshalling content using data " . print_r($data, true));
//
//    $content = array();
//
...
//
//    // TODO
//    $this->log("FOO returning content " . print_r($content, true));
//    return $content;
//  }
//
//  /**
