...
class Oa4mpClientAccessControlsController extends StandardController {
  ...
  function add() {
    ...
    if($this->request->is(array('post','put'))) {
      ...
      if($ret == 0) {
        ...
        $this->Flash->set(_txt('pl.oa4mp_client_access_control.add.flash.success'), array('key' => 'success'));
    ...
    $this->set('title_for_layout', _txt('pl.oa4mp_client_access_control.add.name',
               array(filter_var($client['Oa4mpClientCoOidcClient']['name'], FILTER_SANITIZE_SPECIAL_CHARS))));
  }

  /**
   * Edit an access control configuration.
   *
   * @since  COmanage Registry v4.5.0
   * @param  integer $id Oa4mpClientAccessControl ID
   * @return null
   */
  ...
  function edit($id) {
    ...
    if($this->request->is(array('post','put'))) {
      ...
      if($ret == 0) {
        ...
        // Set flash successful.
        $this->Flash->set(_txt('pl.oa4mp_client_access_control.edit.flash.success'), array('key' => 'success'));
    ...
    $this->set('title_for_layout', _txt('pl.oa4mp_client_access_control.edit.name',
               array(filter_var($client['Oa4mpClientCoOidcClient']['name'], FILTER_SANITIZE_SPECIAL_CHARS))));
  }
  ...
  function index() {
    ...
    $this->set('title_for_layout', _txt('pl.oa4mp_client_access_control.index.name',
               array(filter_var($client['Oa4mpClientCoOidcClient']['name'], FILTER_SANITIZE_SPECIAL_CHARS))));

    // Set page title
    $this->set('vv_oidc_client_name', $client['Oa4mpClientCoOidcClient']['name']);

    // Find all access control configurations for this client
    $args = array();
    $args['conditions']['Oa4mpClientAccessControl.client_id'] = $clientId;
    $args['contain'] = false;

    $this->set('access_controls', $this->Oa4mpClientAccessControl->find('all', $args));
  }
  ...
  function isAuthorized() {
    ...
    // Add a new access control configuration?
    $p['add'] = ($roles['cmadmin'] || $roles['coadmin']);
    
    // Delete an existing access control configuration?
    $p['delete'] = ($roles['cmadmin'] || $roles['coadmin']);
    
    // Edit an existing access control configuration?
    $p['edit'] = ($roles['cmadmin'] || $roles['coadmin']);
    
    // View all existing access control configurations?
