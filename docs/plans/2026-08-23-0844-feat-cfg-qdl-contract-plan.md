...
- **The misleading verdict on read paths.** A contract-read failure no longer reports a client as tampered with on the *edit* path — an internal failure now returns the generic edit error. The read paths in roughly thirteen controllers still map "not synchronized" to "This client has been modified outside of the Registry", so an internal error encountered while viewing a client still misreports. Fixing it means touching every one of those controllers and belongs in its own change.
