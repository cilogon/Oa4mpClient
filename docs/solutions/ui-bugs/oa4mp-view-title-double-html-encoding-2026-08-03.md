...
- The defect was systemic: 14 sites across 9 controllers, every view that embeds the client name in `title_for_layout`.
...
Applied at all 14 sites across 9 controllers:
...
With the controller pre-encoding, `scott's` became `scott&#39;s` (first pass), and the element then encoded the `&` to `&#38;` (`FILTER_SANITIZE_SPECIAL_CHARS` emits numeric entities, not `&amp;`), producing `scott&#38;#39;s`, which a browser displays as the literal `&#39;`. Remove the pre-encode and the element does it once: raw `scott's` -> `scott&#39;s` in the HTML source -> browser renders `scott's`.
...
A useful diagnostic invariant for this class of bug: in raw View-Source, a single `&#39;` is **correct** (the browser renders it as `'`); `&#38;#39;` is the double-encoded, broken form. Grep for `&#38;#39;`, not `&amp;#39;` — `FILTER_SANITIZE_SPECIAL_CHARS` never emits the named entity.
...
- **Regression coverage:** locked by `Test/Case/View/ViewTitleEncodingTest.php`. `testRawNameIsEscapedExactlyOnce` renders the real core `pageTitleAndButtons` element and asserts a single `&#39;` with no `&#38;`; `testPreEncodedNameIsDoubleEncodedByTheElement` renders the failure mode and asserts `&#38;#39;`; `testNoControllerPreEncodesTitleForLayout` scans every `Controller/*.php` statement-wise (not line-wise) so no controller can reintroduce a pre-encode on a `title_for_layout` value; `testTitleScanDetectsAPreEncodedStatement` proves the scan sees across a line break. Run with `Test/run.sh`.
