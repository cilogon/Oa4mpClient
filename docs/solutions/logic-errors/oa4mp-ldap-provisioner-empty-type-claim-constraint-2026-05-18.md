...
2. **Treating the cfg-writer empty-guard at `Model/Oa4mpClientOa4mpServer.php:836-838` as the primary fix site.** That block uses `||` instead of `&&` in its guard — `if(!empty($constraintMapping['constraint_field']) || !empty($constraintMapping['constraint_value']))` — which *would* emit a degenerate `{type: ""}` to the OA4MP server if a row ever reached it. But it was a red herring for this bug: nothing reached it, because the `notBlank` validation on `constraint_value` blocked the writer at site 1 before any row was ever persisted, let alone serialized to cfg. (The `||`-vs-`&&` guard is still suspect and worth a follow-up audit, but it is not what fixed these symptoms.)

A secondary observation also surfaced after the fix landed but is **out of scope** for this learning: the controller migration block at `Controller/Oa4mpClientCoOidcClientsController.php:455` uses a coarse `$alreadyMigrated = !empty($client['Oa4mpClientClaim'])` gate that prevents re-running migration on partially-migrated clients. The user worked around it by resetting the DB with SQL to re-trigger migration end-to-end and confirm the fix. The coarse-gate issue is tracked as a separate follow-up.
...
While confirming the fix, the coarse migration gate at `Controller/Oa4mpClientCoOidcClientsController.php:455` (`!empty($client['Oa4mpClientClaim'])`) prevented re-running migration on a partially-migrated client. To re-exercise the write path end-to-end, reset the DB for the affected client (substitute the actual claim IDs from `cm_oa4mp_client_claims WHERE client_id = <id>`):
