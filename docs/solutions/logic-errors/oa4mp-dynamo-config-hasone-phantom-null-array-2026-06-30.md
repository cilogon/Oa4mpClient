...
- **Gap closed:** the *agreement* between the two call sites, which is what the second commit (`d354872`) actually fixed, is now tested by `Test/Case/Model/ClaimCfgFallbackTest.php`. For a client with no per-client row, it asserts `oa4mpMarshallCfgQdl()` sends the `DefaultDynamoConfig` values and that `isClientDataSynchronized()` then reports in-sync against a server representation carrying those same values. Verified: reverting only the compare-side read to its pre-fix form -- `isClientDataSynchronized()` reading `$curData['Oa4mpClientDynamoConfig']` directly, bypassing `resolveDynamoConfig()` -- reddens exactly that one test and nothing else in the suite.
